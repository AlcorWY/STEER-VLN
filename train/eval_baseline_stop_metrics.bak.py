...
def main():
    ...
    # Load model: local-first, optional remote fallback.
    # Default local path:
    #   /home/wy/uav/OpenFly-Platform/models/openfly-agent-7b
    # Override with:
    #   export OPENFLY_LOCAL_MODEL_PATH=/your/local/openfly-agent-7b
    # Remote fallback is disabled by default for offline reproducibility.
    ...
    if os.path.isdir(local_model_path) and os.path.isfile(local_config):
        ...
        print(f"[Model] Using local OpenFly model: {os.path.abspath(model_name_or_path)}")
        ...
        raise FileNotFoundError(
            ...
            "  1) Put the full openfly-agent-7b model under models/openfly-agent-7b; or\n"
            "  2) export OPENFLY_LOCAL_MODEL_PATH=/absolute/path/to/openfly-agent-7b; or\n"
            "  3) if online is allowed: export HF_FALLBACK_REMOTE=1 and unset offline env vars.\n"
            "Note: a LoRA adapter directory alone is not enough for this baseline eval; "
            "this script needs the full base model directory containing config.json and model shards.\n"
        )
    ...
    for env_name, eval_info in env_groups.items():
        ...
        for idx, item in enumerate(eval_info):
            ...
            if sample_pred_stop and dis < 20:
                successful_stop_count += 1

            if (not sample_pred_stop) and step >= MAX_STEP:
                forced_end_count += 1
